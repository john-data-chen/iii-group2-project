...
# 本程式目的為分析0050在收盤價突破與跌破均線策略下的報酬率
...
maDay = readline()
# 計算均線，收盤價放在第6行
a = runMean(as.numeric(priceXts[,6]),n = as.numeric(maDay))
...
position <- Lag(ifelse(priceXts[,6] > ma, 1,0))
...
# 把所有自訂參數接在一起，方便命名
fileName = paste(maDay, fromDate, sep=",")
# 另存成csv
write.table(df_return, file =sub("%s", fileName, "0050_MA_%s.csv"), sep = ",", col.names = NA)
